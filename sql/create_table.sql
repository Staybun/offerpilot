...
(
    id          bigint auto_increment comment 'id' primary key,
    messageId   varchar(128)                       not null comment '消息唯一 id',
    messageType varchar(64)                        not null comment '消息类型',
    status      tinyint  default 1                 not null comment '状态：1-已消费',
    createTime  datetime default CURRENT_TIMESTAMP not null comment '创建时间',
    updateTime  datetime default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP comment '更新时间',
    unique key uk_messageId (messageId),
    index idx_messageType (messageType)
    ) comment 'MQ 消息消费记录' collate = utf8mb4_unicode_ci;
